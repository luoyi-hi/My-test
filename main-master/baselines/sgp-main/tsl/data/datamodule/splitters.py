...
class MyTimeStepSplitter(Splitter):
    ...
    def __init__(
        ...
        super(MyTimeStepSplitter, self).__init__()
        self.first_val_ts = first_val_ts
        self.first_test_ts = first_test_ts
        self.last_val_ts = last_val_ts
        self.last_test_ts = last_test_ts
    ...
    def fit(self, dataset: SpatioTemporalDataset):
        ...
        train_idx = train_idx - self.input_len - self.output_len + 1
        val_idx = val_idx - self.input_len - self.output_len + 1
        test_idx = test_idx - self.input_len - self.output_len + 1
        return self.set_indices(train_idx, val_idx, test_idx)
...
class AtTimeStepSplitter3(Splitter):
    ...
    def __init__(
        ...
        super(AtTimeStepSplitter3, self).__init__()
        self.first_val_ts = first_val_ts
        self.first_test_ts = first_test_ts
        self.last_val_ts = last_val_ts
        self.last_test_ts = last_test_ts
    ...
    def fit(self, dataset: SpatioTemporalDataset):
        train_idx = np.arange(0, 15711)
        val_idx = np.arange(15711, 20948)
        test_idx = np.arange(20948, 26185)
        return self.set_indices(train_idx, val_idx, test_idx)
...
def disjoint_months(dataset, months=None, synch_mode=SynchMode.WINDOW):
    ...
    if synch_mode is SynchMode.WINDOW:
        ...
        raise ValueError(
            "synch_mode can only be one of %s" % [SynchMode.WINDOW, SynchMode.HORIZON]
        )
    # after idxs
