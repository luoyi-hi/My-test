...
#         mapping = {un: pd.to_timedelta('1' + un)
#            for un in ['day', 'hour', 'minute', 'second',
#                       'millisecond', 'microsecond', 'nanosecond']}
#         mapping['week'] = pd.to_timedelta('1W')
...
class TemporalFeaturesMixin:
    ...
    def datetime_encoded(self, units):
        ...
        # 添加 'year' 单位的映射（基于平均年份长度）
        mapping['year'] = int(365.2425 * 24 * 60 * 60 * 1e9)  # 365.2425 天
        ...
        for unit in units:
            if unit not in mapping:
                raise ValueError(f"Unit '{unit}' not in mapping")
